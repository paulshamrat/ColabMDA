...
def parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Frame interval in ps. Effective = sim_interval * stride",
    )
...
def main():
    ...
    if interval is None:
        try:
            ...
            source = "DCD header (WARNING: likely inaccurate)"
            print(
                "!! WARNING: Using DCD header interval. If total time looks wrong, pass --interval manually."
            )
        except Exception:
            interval = None
    if interval is None:
