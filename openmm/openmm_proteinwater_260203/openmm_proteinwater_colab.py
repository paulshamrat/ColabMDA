...
def parse_args():
    ...
    p.add_argument(
        "--sync-dir", default=None, help="If set, sync outputs to this directory after each chunk"
    )
    return p.parse_args()
...
def setup_and_equilibrate(cleaned_pdb, dt, eq_steps, xml_sys, pdb_saved, chk_file):
    ...
    sim.reporters.append(
        StateDataReporter("nvt.log", max(1, eq_steps // 10), step=True, temperature=True)
    )
    sim.step(eq_steps)
    sim.reporters.clear()
    ...
    sim.reporters.append(
        StateDataReporter(
            "npt.log", max(1, eq_steps // 10), step=True, temperature=True, volume=True
        )
    )
    sim.step(eq_steps)
    sim.reporters.clear()
