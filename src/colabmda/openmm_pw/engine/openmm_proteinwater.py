...
def setup_and_equilibrate(cleaned_pdb, dt, eq_steps, xml_sys, pdb_saved, chk_file):
    ...
    sim.reporters.append(
        StateDataReporter("npt.log", eq_steps // 10, step=True, temperature=True, volume=True)
    )
    sim.step(eq_steps)
    sim.reporters.clear()
    ...
    PDBFile.writeFile(
        sim.topology, sim.context.getState(getPositions=True).getPositions(), open(pdb_saved, "w")
    )
