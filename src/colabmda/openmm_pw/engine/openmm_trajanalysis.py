...
def run_trajectory_analysis(pdbid, topology=None, trajectory=None, interval=None, outdir=None):
    ...
    # Plotting Styles
    plt.rcParams.update(
        {"font.size": 12, "figure.figsize": (10, 6), "axes.grid": True, "grid.alpha": 0.3}
    )
    ...
    for i, res in enumerate(results):
        ...
        pd.DataFrame({"Time (ps)": res["times"], "RMSD (Å)": res["rmsd"]}).to_csv(
            target / "rmsd.csv", index=False
        )
        pd.DataFrame({"Time (ps)": res["times"], "Rg (Å)": res["rg"]}).to_csv(
            target / "rg.csv", index=False
        )
        pd.DataFrame({"Residue": res["resids"], "RMSF (Å)": res["rmsf"]}).to_csv(
            target / "rmsf.csv", index=False
        )

        # RMSD
        plt.figure()
        plt.plot(res["times"], res["rmsd"], color=colors[0])
        plt.xlabel("Time (ps)")
        plt.ylabel("RMSD (Å)")
        plt.title(f"Backbone RMSD - {labels[i]}")
        plt.savefig(target / "rmsd_vs_time.png", dpi=300)
        plt.close()

        # Rg
        plt.figure()
        plt.plot(res["times"], res["rg"], color=colors[1])
        plt.xlabel("Time (ps)")
        plt.ylabel("Rg (Å)")
        plt.title(f"Radius of Gyration - {labels[i]}")
        plt.savefig(target / "rg_vs_time.png", dpi=300)
        plt.close()

        # RMSF
        plt.figure()
        plt.plot(res["resids"], res["rmsf"], color=colors[2])
        plt.xlabel("Residue Index")
        plt.ylabel("RMSF (Å)")
        plt.title(f"Residue Flexibility - {labels[i]}")
        plt.savefig(target / "rmsf_per_residue.png", dpi=300)
        plt.close()
    ...
    if len(results) > 1:
        ...
        # Save Aggregate CSV
        pd.DataFrame({"Time (ps)": t_avg, "Mean": avg_rmsd, "Std": std_rmsd}).to_csv(
            agg_dir / "rmsd_avg.csv", index=False
        )
        ...
        plt.ylabel("RMSD (Å)")
        plt.legend()
        plt.title("Aggregate Backbone RMSD")
        plt.savefig(agg_dir / "rmsd_aggregate.png", dpi=300)
        plt.close()
        ...
        # Save Aggregate CSV
        pd.DataFrame({"Time (ps)": t_avg, "Mean": avg_rg, "Std": std_rg}).to_csv(
            agg_dir / "rg_avg.csv", index=False
        )
        ...
        plt.ylabel("Rg (Å)")
        plt.legend()
        plt.title("Aggregate Radius of Gyration")
        plt.savefig(agg_dir / "rg_aggregate.png", dpi=300)
        plt.close()
        ...
        # Save Aggregate CSV
        pd.DataFrame({"Residue": resids, "Mean": avg_rmsf, "Std": std_rmsf}).to_csv(
            agg_dir / "rmsf_avg.csv", index=False
        )
        ...
        plt.ylabel("RMSF (Å)")
        plt.legend()
        plt.title("Aggregate Residue Flexibility")
